...
<<<<<<< HEAD
	...
	cout << thread_bound[numThreads - 1] << endl;
...
#pragma omp barrier
			...
			for (Ipp32u i = 1; i < nb; ++i){
				...
				Ipp32s stepR2 = v2[0];
				float prime_temp = prime_log[ind];
				if (primePower <= M){
					//sieving by first root
					while (stepR1 <= M){
						sieve[thread_id][stepR1 + M] -= prime_temp;
						stepR1 += primePower;
=======
...
>>>>>>> parent of dbd1cb3... Works well with tbb
					...
					stepR1 = v[0] + M - primePower;
					while (stepR1 >= 0){
						sieve[thread_id][stepR1] -= prime_temp;
						stepR1 -= primePower;
					}
					//sieving by second root
					while (stepR2 <= M){
						sieve[thread_id][stepR2 + M] -= prime_temp;
						stepR2 += primePower;
					}
					stepR2 = v2[0] + M - primePower;
					while (stepR2 >= 0){
						sieve[thread_id][stepR2] -= prime_temp;
						stepR2 -= primePower;
					}
				}
			} //end for one prime from factor base
			...
			for (Ipp32s i = 0; i < 2 * M + 1; ++i){
				sieve[thread_id][i] += Q(BigNumber(i - M)).b_ln();
			}
			...
			for (Ipp32s i = 0; i < 2 * M + 1 & thread_counter[thread_id] < thread_bound[thread_id]; ++i){
				if (sieve[thread_id][i] <= epsilon){
					...
					//Trial division stage
					Ipp32u res_ind = thread_begin_index[thread_id] + thread_counter[thread_id];
					if (Qx > BigNumber::Zero()){
						for (auto jt = Base.begin(); jt != Base.end(); ++jt){
							...
							result[res_ind].second[ind] = deg;
						}
						if (Qx == BigNumber::One()){
							result[res_ind].first = (A*x + B) * Q1; //???
							++thread_counter[thread_id];
						}
						else{
							for (auto& kk : result[res_ind].second)  //I want to zero all degrees
								kk = 0;
						}
					}
				}
